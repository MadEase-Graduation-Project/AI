...
from config import (
    TRAINING_DATA_PATH, 
    # TESTING_DATA_PATH,  # Commented out - not used
    SYMPTOM_SEVERITY_PATH,
    SYMPTOM_DESCRIPTION_PATH,
    SYMPTOM_PRECAUTION_PATH,
    TEST_SIZE,
    RANDOM_STATE,
    # TRAINING_AUGMENTED_DATA_PATH,  # Commented out - file doesn't exist
...
class DataPreprocessorFixed:
    ...
    def load_datasets(self, use_augmented=True, use_ai_augmented=False, use_safe_augmented=False):
        """Load Datasets for training and testing"""
        try:
            if use_safe_augmented:
                self.training = pd.read_csv(TRAINING_SAFE_AUGMENTED_DATA_PATH)
                print("✓ Using safe augmented dataset")
            elif use_ai_augmented:
                self.training = pd.read_csv(TRAINING_AI_AUGMENTED_DATA_PATH)
                print("✓ Using AI-augmented dataset")
            elif use_augmented:
                # Fallback to original dataset since augmented file doesn't exist
                print("⚠️  Augmented dataset not found, using original dataset")
    ...
    def initialize_all(self, use_augmented=True, use_ai_augmented=False, use_safe_augmented=False):
        """Initialize all components of the data preprocessor"""
        print("🔄 Initializing data preprocessor...")
        
        # Load datasets
        self.load_datasets(use_augmented, use_ai_augmented, use_safe_augmented)
        ...
        print("✅ Data preprocessor initialization complete!")
        return self 